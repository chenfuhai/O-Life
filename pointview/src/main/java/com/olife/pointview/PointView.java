...
public class PointView extends View {
    ...
    public PointView(Context context, AttributeSet attrs) {
        ...
        TypedArray typedArray = getResources().obtainAttributes(attrs, R.styleable.PointView);

        mProgessMax = typedArray.getInteger(R.styleable.PointView_score_max, 100);
        mProgessCurr = mProgessMax;

        mWaitingStrokeWidth = typedArray.getDimension(R.styleable.PointView_waiting_strokewidth,
                TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 5, getResources().getDisplayMetrics()));
        mWaitingPointCount = typedArray.getInteger(R.styleable.PointView_waiting_pointcount, 48);
        mWaitingPointColor = typedArray.getColor(R.styleable.PointView_waiting_pointcolor, Color.WHITE);

        mTextScoreSize = typedArray.getDimension(R.styleable.PointView_score_size,
                TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 50, getResources().getDisplayMetrics()));
        mTextScoreColor = typedArray.getColor(R.styleable.PointView_score_fontcolor, Color.WHITE);
        mTextScore = mProgessCurr + "";

        mTextUnitSize = mTextScoreSize / 3f;
        mTextUnitColor = typedArray.getColor(R.styleable.PointView_unit_fontcolor, Color.WHITE);

        if (typedArray.getString(R.styleable.PointView_unit_text) != null) {
            mTextUnit = typedArray.getString(R.styleable.PointView_unit_text);
        } else {
            mTextUnit = "分";
        }

        if (typedArray.getString(R.styleable.PointView_tip_before) != null) {
            mTextTipStart = typedArray.getString(R.styleable.PointView_tip_before);
        } else {
            mTextTipStart = "点击立即检测";
        }
        if (typedArray.getString(R.styleable.PointView_tip_doing) != null) {
            mTextTipDoing = typedArray.getString(R.styleable.PointView_tip_doing);
        } else {
            mTextTipDoing = "检测中...";
        }
        if (typedArray.getString(R.styleable.PointView_tip_done) != null) {
            mTextTipStop = typedArray.getString(R.styleable.PointView_tip_done);
        } else {
            mTextTipStop = "检测完成";
        }
        mTextTip = mTextTipStart;
        mTextTipSize = mTextScoreSize / 3f;
        mTextTipColor = typedArray.getColor(R.styleable.PointView_tip_fontcolor, Color.WHITE);
    ...
    private void drawMyText(Canvas canvas) {
        ...
        float stratY = getHeight() / 2 + mOvalTextScore.height() / 2 - dip2px(5);
        ...
        float stratY2 = getHeight() / 2 + mOvalTextScore.height() / 2 - dip2px(6);
        ...
        float stratY3 = getHeight() / 2 + mOvalTextTip.height() / 2 + mOvalTextScore.height() / 2 + dip2px(10);
    ...
    private void initData() {
        ...
        mOvalWaitting.top = (getHeight() / 2 - mRadiusPoint);
        mOvalWaitting.right = (getWidth() / 2 + mRadiusPoint);
        mOvalWaitting.bottom = (getHeight() / 2 + mRadiusPoint);
